...
func UpdateTransactionPayment(transactionUpdate *models.Notification) error {
	...
	if err := config.DB.Model(&transaction).Where("transaction_number = ?", transactionUpdate.OrderID).Updates(models.Transaction{
		PaymentStatus: transactionUpdate.TransactionStatus, PaymentMethod: transactionUpdate.PaymentType}).Error; err != nil {
		return err
	}
